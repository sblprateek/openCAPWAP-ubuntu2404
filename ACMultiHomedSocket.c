...
CWBool CWNetworkInitSocketServerMultiHomed(CWMultiHomedSocket *sockPtr, 
	...
	for (ifihead = ifi = get_ifi_info_new((gNetworkPreferredFamily == CW_IPv6) ? AF_INET6 : AF_INET, 1); ifi != NULL; ifi = ifi->ifi_next) { 
#else
	/* for each network interface... */
	for (ifihead = ifi = get_ifi_info_new((gNetworkPreferredFamily == CW_IPv6) ? AF_INET6 : AF_INET, 0); ifi != NULL; ifi = ifi->ifi_next) {
#endif
		...
		if(bind(sock, (struct sockaddr*) ifi->ifi_addr, CWNetworkGetAddressSize((CWNetworkLev4Address*)ifi->ifi_addr)) < 0) {
			...
			close(sock);
			CWDebugLog("Failed to bind on %s!", ifi->ifi_name);
		...
		if(CWNetworkGetInterfaceAlreadyStored(interfaceList, ifi->ifi_index) == NULL &&
		   strncmp(ifi->ifi_name, "lo", 2)) { /* don't consider loopback an interface
			...
			CWDebugLog("Marking %s (index %d) as PRIMARY", ifi->ifi_name, ifi->ifi_index);
			//CWDebugLog("Primary Address");
			p->kind = CW_PRIMARY;

		} else {
			/* should be BROADCAST_OR_ALIAS_OR_MULTICAST_OR_LOOPBACK ;-) */
			CWDebugLog("Marking %s (index %d) as BROADCAST_OR_ALIAS", ifi->ifi_name, ifi->ifi_index);
	...
	if(gNetworkPreferredFamily == CW_IPv6) {
		 ...
		 */
#ifdef CW_DEBUGGING
		for (ifihead = ifi = get_ifi_info_new(AF_INET, 1); ifi != NULL; ifi = ifi->ifi_next) {
#else
		for (ifihead = ifi = get_ifi_info_new(AF_INET, 0); ifi != NULL; ifi = ifi->ifi_next) {
#endif
