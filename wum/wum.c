...
int main(int argc, char *argv[])
{
    ...
    while ((c = getopt (argc, argv, "ha:p:w:c:f:n:s:r:l:t:")) != -1)
        ...
        {
		case 'a':
			acserver_address = strdup(optarg);
			break;
		case 'p':
			acserver_port = atoi(optarg);
			break;
        case 'w':
            wtpIds = strdup(optarg);
            break;
        case 'n':
        	wtpNames = strdup(optarg);
        	break;
        case 'c':
            command = strdup(optarg);
            break;
        case 'f':
        	cup_path = strdup(optarg);
        	break;
/* +++ Elena Agostini - 09/2014: IEEE 802.11 Binding. WLAN Commands +++ */
        case 's':
        	...
        	break;
        case 'r':
        	radioID = strdup(optarg);
        	break;
        case 'l':
        	wlanID = strdup(optarg);			
        	break;
		case 't':
        	tunnel = strdup(optarg);
        	break;
/* ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
        case 'h':
        	usage(argv[0]);
...
void do_version_cmd(int acserver, char *wtpIds, char *wtpNames)
{
	...
	fprintf(stderr, "WTPs to check: %d\n", n);
