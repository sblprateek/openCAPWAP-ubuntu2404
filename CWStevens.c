...
struct ifi_info* get_ifi_info_new(int family, int doaliases)
{
struct ifi_info     *head = NULL, **next_p;
    struct ifaddrs      *ifap, *ifa;
    char                lastname[IFI_NAME] = {0};
    char                current_name[IFI_NAME];
    char                *cptr;
    size_t              addr_len;

    // CWLog("get_ifi_info_modern(%x, %x)", family, doaliases);
    ...
    for (ifa = ifap; ifa != NULL; ifa = ifa->ifa_next) {
        ...
        /* 2. 名称处理 (模拟原代码逻辑：去除 : 及其后缀) */
        /* 这里的目的是把 eth0:1 处理成 eth0，以便进行别名判断 */
        memset(current_name, 0, IFI_NAME);
        strncpy(current_name, ifa->ifa_name, IFI_NAME - 1);
        
        if ((cptr = strchr(current_name, ':')) != NULL) {
            *cptr = 0; /* 截断冒号后的内容 */
        }

        /* 3. 别名过滤逻辑 */
        /* 如果当前接口名与上一次处理的接口名相同，且不允许别名，则跳过 */
        if (strncmp(lastname, current_name, IFI_NAME) == 0) {
            if (doaliases == 0)
                continue;
        }
        
        /* 更新 lastname 用于下一次比较 */
        memcpy(lastname, current_name, IFI_NAME);
        ...
        *next_p = ifi;
        ...
        /* 5. 填充数据 */
        ifi->ifi_flags = (short)ifa->ifa_flags;
        ifi->ifi_index = (short)if_nametoindex(ifa->ifa_name);
        
        /* 注意：这里我们要复制处理过的 current_name (即去掉了 :1 的名字) 
         * 这样保持与原代码行为一致 */
        memcpy(ifi->ifi_name, current_name, IFI_NAME); 
        ...
        /* 6. 复制 IP 地址 */
        ifi->ifi_addr = (struct sockaddr*)calloc(1, addr_len);
        if (ifi->ifi_addr) {
            memcpy(ifi->ifi_addr, ifa->ifa_addr, addr_len);
        }

        /* 7. 复制广播地址 (仅当接口支持广播且地址存在时) */
        if ((ifi->ifi_flags & IFF_BROADCAST) && ifa->ifa_broadaddr != NULL) {
            ifi->ifi_brdaddr = (struct sockaddr*)calloc(1, addr_len);
            if (ifi->ifi_brdaddr) {
                memcpy(ifi->ifi_brdaddr, ifa->ifa_broadaddr, addr_len);
            }
        }
    }
...
struct ifi_info* get_ifi_info(int family, int doaliases)
{
	...
	CWLog("get_ifi_info(%x, %x)", family, doaliases);
...
#endif
		CWLog("Processing ifr_name='%s', ifr_newname='%s', ifr_slave='%s', sa_family %x", 
			ifr->ifr_name, ifr->ifr_newname,  ifr->ifr_slave, ifr->ifr_addr.sa_family);
